...
select 
        ...
        coalesce(
        	(case 
        	      ...
        	      (t.hero_stars / (t.hero_Score + 1) / SUM(hero_stars::float / (hero_score + 1)) OVER (PARTITION BY t.tournament_player_deck_id) )   * (coalesce(eth.reward,0) + (frags.reward / 100 * 0.0017))
--        	      	case when t.player_score = 0 then eth.reward / 5
--        	 	    else  
--        	 	    (1 - (hero_score::decimal / NULLIF(player_score, 0)::decimal)) / 
--         				SUM(1 - (hero_score::decimal / NULLIF(player_score, 0)::decimal)) OVER(partition by tournament_player_deck_id) * (eth.reward + (frags.reward / 100 * 0.0017))
--        	 	    end
        	 	  when t2.league <>  'Reverse'  then
        	 	  	case when t.hero_score  = 0 then 0 
					else (hero_score::float / t.player_score) * (coalesce(eth.reward,0) + (frags.reward / 100 * 0.0017))
					end
